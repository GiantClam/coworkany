...
桌面端默认使用固定右侧 AI 侧栏，宽度 `360-420px`，最小宽度 `320px`，支持拖拽调整。侧栏不覆盖节点，画布根据侧栏宽度重新计算可视区域。窄屏时侧栏退化为右侧可拖拽抽屉，默认约占屏幕 `88%`，并提供“定位到刚修改节点”操作。
...
首次打开空白画布时，显示“从一句话创建”“从模板开始”“分析当前画布”三个入口。AI 回复必须给出可理解的变更摘要，而不是只显示“完成”。
