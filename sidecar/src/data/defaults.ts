...
0. **标的消歧（必须先做）**：
   - 先确认股票唯一标识：ticker + 交易所（如 NVDA (NASDAQ)、0700.HK (HKEX)）。
   - 如果只有公司名且可能同名（如 MiniMax/GLM 等），先用 search_web("[公司名] ticker exchange") 消歧。
   - 未确认唯一标的前，不要直接下结论“今日涨跌/暴涨原因”。
...
- **风险因素**: 竞争、监管、估值过高
- **技术面参考**: 股价趋势、52周高低点
- **时间锚点**: 涨跌数据必须标注交易日、时区、盘中/收盘时间点（避免“今天”歧义）
...
- 如果新闻条目大面积出现 No title / Unknown source：判定为低质量源，立即改用 search_web + crawl_url 抓取可读来源再分析
