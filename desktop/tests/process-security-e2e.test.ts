...
// Query contains '检查' and '分析' — these should trigger post-execution learning
...
test.describe('本机进程安全分析 — 自学习场景 E2E', () => {
    ...
    test('通过桌面客户端检查本机进程安全风险，验证自学习技能沉淀', async ({ page, tauriLogs }) => {
        ...
        while (Date.now() - startTime < TASK_TIMEOUT_MS) {
            ...
            if (!runCommandDetected && tauriLogs.contains('run_command')) {
                ...
                console.log(`[Test] [${elapsed}s] 🖥️ run_command tool detected`);
            }
            ...
            if (!securityAnalysisDone) {
                ...
                if (textLines.some(l => securityKeywords.some(kw => l.includes(kw)))) {
                    ...
                    console.log(`[Test] [${elapsed}s] 🛡️ Security analysis detected in response`);
                }
            }
        ...
        console.log('  本机进程安全分析 + 自学习 E2E 测试报告 (Tauri Desktop)');
        console.log('='.repeat(70));
        ...
        console.log('');
        console.log('  ── 任务规划 (持久化增强) ──');
        console.log(`  plan_step使用: ${planStepUsed ? 'YES ✅' : 'NO ⚠️'}`);
        console.log(`  计划持久化: ${planPersisted ? 'YES ✅ (task_plan.md)' : 'NO ⚠️'}`);
        console.log(`  log_finding使用: ${logFindingUsed ? 'YES ✅ (findings.md)' : 'NO ⚠️'}`);
        console.log(`  计划上下文注入: ${planContextInjected ? 'YES ✅ (PreToolUse)' : 'NO ⚠️'}`);
        ...
        expect(processListRequested, '应该执行了进程列表命令(tasklist/Get-Process/wmic)').toBe(true);
        ...
        expect(securityAnalysisDone, '应提供进程安全分析').toBe(true);
        expect(taskSucceeded, '进程安全分析任务应成功完成').toBe(true);
