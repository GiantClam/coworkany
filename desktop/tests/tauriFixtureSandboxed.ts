/**
 * Tauri fixture with isolated CoworkAny app data.
 *
 * This launches the desktop app with COWORKANY_APP_DATA_DIR pointing to a
 * temporary directory so GUI E2E tests can safely exercise self-management
 * flows that read/write llm-config.json or workspace metadata.
 */
    ...
    while (Date.now() - startTime < timeoutMs) {
        try {
            ...
            if (res.ok) {
                ...
                console.log(`[Fixture-Sandboxed] CDP ready on port ${port}: ${data['Browser'] || 'unknown'}`);
                return true;
            }
        } catch {
            // Not ready yet
...
export const test = base.extend<TauriFixtures>({
    appDataDir: [async ({}, use, testInfo) => {
        const appDataDir = fs.mkdtempSync(
            path.join(fs.realpathSync(os.tmpdir()), `coworkany-sandboxed-appdata-${testInfo.workerIndex}-`)
        );
    ...
    tauriProcess: [async ({ appDataDir }, use, testInfo) => {
        console.log(`[Fixture-Sandboxed] Starting Tauri app with isolated appDataDir=${appDataDir}`);
        ...
        if (process.platform === 'win32') {
            ...
            env.WEBVIEW2_USER_DATA_FOLDER = path.join(
                ...
                `coworkany-sandboxed-webview-${testInfo.workerIndex}`,
            );
        ...
        console.log(`[Fixture-Sandboxed] Tauri process spawned (PID: ${tauriProc.pid})`);

        await use(tauriProc);

        console.log(`[Fixture-Sandboxed] Killing Tauri process tree (PID: ${tauriProc.pid})...`);
        try {
            if (process.platform === 'win32') {
    ...
    context: [async ({ playwright }, use) => {
        console.log(`[Fixture-Sandboxed] Waiting for CDP on port ${CDP_PORT}...`);
