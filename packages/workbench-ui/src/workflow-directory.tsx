...
export function WorkbenchWorkflowDirectory({ locale, workflows, templates, recentRuns, onAction, actionAvailability }: {
  ...
  return <div className="wb-workflow-directory" data-cloud-surface="workflow-directory">
    ...
    <section className="wb-workflow-section" data-workflow-section="templates"><header><div><div className="wb-workflow-eyebrow">{copy.templateEyebrow}</div><h2>{copy.template}</h2><p>{copy.templateDescription}</p></div><strong>{templates.length}</strong></header>{templates.length ? <div className="wb-workflow-template-grid">{templates.map((template) => <article key={template.id} data-template-status={template.status}><div className="wb-workflow-card-heading"><h3>{template.title}</h3><span>{template.status}</span></div><p>{template.description}</p><button type="button" disabled={template.status === "unavailable"} onClick={() => void onAction({ type: "instantiate", id: template.id })}>{copy.instantiate}</button></article>)}</div> : <div className="wb-workflow-empty">{copy.noTemplates}</div>}</section>
