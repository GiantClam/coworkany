...
test("product promotion template splits once, expands scenes, and embeds a cover", () => {
  ...
  assert.deepEqual(definition.nodes.map((node) => node.type), ["text_input", "image_generate", "text_split", "foreach", "video_generate", "collect", "video_process", "image_generate", "video_compose", "output"]);
  assert.equal(definition.nodes.filter((node) => node.type === "text_split").length, 1);
  assert.equal(definition.nodes.filter((node) => node.type === "foreach").length, 1);
  assert.equal(definition.nodes.filter((node) => node.type === "video_generate").length, 1);
  assert.equal(definition.nodes.filter((node) => node.type === "collect").length, 1);
  const scriptText = definition.nodes.find((node) => node.nodeKey === "script")?.config.text;
  assert.equal(splitWorkflowText(typeof scriptText === "string" ? scriptText : "", "heading").length, 6);
  const scene = definition.nodes.find((node) => node.nodeKey === "scene-video");
  assert.equal(scene?.config.mode, "reference-to-video");
  assert.equal(scene?.config.sound, "on");
  assert.equal(scene?.config.ratio, "9:16");
  assert.deepEqual(definition.edges.find((edge) => edge.edgeKey === "split-foreach"), { edgeKey: "split-foreach", sourceNodeKey: "script-split", sourcePortId: "texts", targetNodeKey: "scene-foreach", targetPortId: "items.text" });
  assert.deepEqual(definition.edges.find((edge) => edge.edgeKey === "reference-foreach"), { edgeKey: "reference-foreach", sourceNodeKey: "creator-reference", sourcePortId: "image", targetNodeKey: "scene-foreach", targetPortId: "referenceImage" });
  assert.deepEqual(definition.edges.find((edge) => edge.edgeKey === "foreach-scene"), { edgeKey: "foreach-scene", sourceNodeKey: "scene-foreach", sourcePortId: "item.text", targetNodeKey: "scene-video", targetPortId: "text" });
  assert.deepEqual(definition.edges.find((edge) => edge.edgeKey === "foreach-reference-scene"), { edgeKey: "foreach-reference-scene", sourceNodeKey: "scene-foreach", sourcePortId: "item.image", targetNodeKey: "scene-video", targetPortId: "referenceImages" });
  assert.deepEqual(definition.edges.find((edge) => edge.edgeKey === "scene-collect"), { edgeKey: "scene-collect", sourceNodeKey: "scene-video", sourcePortId: "video", targetNodeKey: "scene-collect", targetPortId: "items.video" });
  assert.deepEqual(definition.edges.find((edge) => edge.edgeKey === "collect-stitch"), { edgeKey: "collect-stitch", sourceNodeKey: "scene-collect", sourcePortId: "videos", targetNodeKey: "stitch", targetPortId: "videos" });
  assert.equal(definition.nodes.find((node) => node.nodeKey === "stitch")?.config.operation, "stitch");
  assert.equal(definition.nodes.find((node) => node.nodeKey === "stitch")?.config.ratio, "9:16");
  ...
  assert.equal(definition.edges.filter((edge) => edge.targetNodeKey === "stitch" && edge.targetPortId === "videos").length, 1);
  assert.deepEqual(definition.edges.find((edge) => edge.edgeKey === "reference-cover"), { edgeKey: "reference-cover", sourceNodeKey: "creator-reference", sourcePortId: "image", targetNodeKey: "cover-image", targetPortId: "referenceImage" });
  ...
  assert.deepEqual(definition.edges.find((edge) => edge.edgeKey === "stitch-compose"), { edgeKey: "stitch-compose", sourceNodeKey: "stitch", sourcePortId: "video", targetNodeKey: "compose", targetPortId: "videos" });
