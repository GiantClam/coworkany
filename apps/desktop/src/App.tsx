...
const workflowActions: Array<{ id: WorkflowAction; label: string; output: "text" | "asset" | "ppt" | "image" | "video" | "audio" }> = [
  ...
  { id: "video_compose", label: "视频合成", output: "video" },
...
type DesktopWorkflowWorkspaceProps = {
  ...
  agentOptions?: readonly WorkflowDynamicOption[];
  loadVoicesForProvider?: (provider?: DesktopProviderConfig) => Promise<readonly MiniMaxVoiceOption[]>;
  reasoningEffort: string;
  skillId: SkillId;
  onModelChange: (value: string) => void;
  onReasoningChange: (value: string) => void;
...
function DesktopWorkflowCanvas({
  ...
  voiceOptions?: readonly WorkflowDynamicOption[];
  agentOptions?: readonly WorkflowDynamicOption[];
  ...
  return <WorkbenchWorkflowCanvas
    ...
    renderNodeEditor={onUpdateNodeParameter ? (node) => {
      ...
      return <WorkbenchWorkflowParameterFields className="desktop-workflow-node-editor" locale={locale} node={node as WorkflowDefinitionNodeV2} modelOptions={nodeModelOptions} modelLabel={modelLabel} hideWorkflowReference={hideWorkflowReference} providerOptions={providerOptions} voiceOptions={voiceOptions} agentOptions={agentOptions} voiceOptionsLoading={voiceOptionsLoading} voiceOptionsError={voiceOptionsError} onUpdate={(key, value) => onUpdateNodeParameter(node.nodeKey, key, value)} />;
    } : undefined}
  />;
}
...
function runningHubWorkflowCapabilityForNode(node: WorkflowDefinitionNodeV2 | undefined): RunningHubWorkflowCapability | undefined {
  ...
  if (node.type === "agent_execute" && node.config.operation === "audio_transcription") return "audio_transcription";
...
function DesktopWorkflowBuilderSurface(props: WorkflowBuilderSurfaceProps) {
  ...
  const [workflowVoiceOptions, setWorkflowVoiceOptions] = useState<MiniMaxVoiceOption[]>([]);
  const [workflowVoiceOptionsLoading, setWorkflowVoiceOptionsLoading] = useState(false);
  const [workflowVoiceOptionsError, setWorkflowVoiceOptionsError] = useState<string | null>(null);
  ...
  const selectedProviderId = typeof selectedNode?.config.selectedProviderId === "string"
    ? selectedNode.config.selectedProviderId.trim()
    : typeof selectedNode?.config.provider === "string"
      ? selectedNode.config.provider.trim()
      : "";
  ...
  useEffect(() => {
    if (selectedNode?.type !== "voice_synthesis" || !props.loadVoicesForProvider) {
      setWorkflowVoiceOptions([]);
      setWorkflowVoiceOptionsError(null);
      setWorkflowVoiceOptionsLoading(false);
      return;
    }
    let active = true;
    setWorkflowVoiceOptionsLoading(true);
    setWorkflowVoiceOptionsError(null);
    void props.loadVoicesForProvider(selectedProvider)
      .then((voices) => { if (active) setWorkflowVoiceOptions([...voices]); })
      .catch((error) => { if (active) { setWorkflowVoiceOptions([]); setWorkflowVoiceOptionsError(error instanceof Error ? error.message : (locale === "zh" ? "音色加载失败" : "Unable to load voices")); } })
      .finally(() => { if (active) setWorkflowVoiceOptionsLoading(false); });
    return () => { active = false; };
  }, [locale, props.loadVoicesForProvider, selectedNode?.type, selectedProvider?.id]);
...
function DesktopWorkflowWorkspace({ route, onBack, prompt: _prompt, onPromptChange: _onPromptChange, runStatus: _runStatus, activeRunId: _activeRunId, onRun: _onRun, onRerun: providedOnRerun, onContinue: providedOnContinue, onCancel, lastRunStatus, savedWorkflows, workflowAction, onWorkflowAction, definition, onDefinitionChange, workflowMetadata: initialWorkflowMetadata, onWorkflowMetaChange, onSave, onExport, onImport, model, models, modelForNode, modelsForNode, providersForNode, agentOptions, loadVoicesForProvider, reasoningEffort, skillId, onModelChange, onReasoningChange, onSkillChange = onReasoningChange, providerConfiguredForNode, providerSourceForNode, onSelectWorkflowFiles, nodeExecutionSnapshots, locale }: DesktopWorkflowWorkspaceProps & { locale: "zh" | "en" }) {
  ...
  const updateNodeConfig = (nodeKey: string, key: string, value: WorkflowParameterValue) => {
    ...
    if (node.type === "agent_execute" && key === "operation" && nextValue === "audio_transcription") {
      const transcriptionProvider = providersForNode?.(node.type).find((candidate) => candidate.source?.trim().toLowerCase() === "runninghub" && candidate.workflows?.some((workflow) => workflow.capability === "audio_transcription"));
      const transcriptionWorkflow = transcriptionProvider?.workflows?.find((workflow) => workflow.capability === "audio_transcription");
      if (transcriptionProvider?.id) {
        nextConfig.selectedProviderId = transcriptionProvider.id;
        nextConfig.provider = transcriptionProvider.id;
        nextConfig.selectedModelId = transcriptionWorkflow?.remoteWorkflowId ?? preferredConfiguredModel(transcriptionProvider);
        nextConfig.model = nextConfig.selectedModelId;
      }
    }
    if (key === "selectedProviderId" && typeof nextValue === "string") {
      ...
      if (node.type === "voice_synthesis" && nextValue !== node.config.selectedProviderId) delete nextConfig.voiceId;
    }
...
function DesktopLibraryWorkspace({ route, artifactRows, savedWorkflows, conversations, runs, taskCount, tokenCount, artifactCount, providerCost: initialProviderCost, estimatedCost: initialEstimatedCost, onNavigate: navigatePath, onRetryRun, onInspectRun, onArtifactRemove, onArtifactReveal, onKnowledgeOpen, knowledgeQuery, knowledgeResults, knowledgeStatus, onKnowledgeQueryChange, onKnowledgeSearch, locale }: { route: DesktopRoute; artifactRows: Array<ArtifactRow>; savedWorkflows: Array<{ id: string; name: string; definition_json: string; updated_at: string }>; conversations: Array<{ id: string; title: string; updated_at: string; agent_id?: string | null }>; runs: RunRow[]; taskCount: number; tokenCount: number; artifactCount: number; providerCost?: number; estimatedCost?: number; onNavigate: (path: string) => void; onRetryRun: (run: RunRow) => void; onInspectRun: (runId: string) => Promise<RunDetail>; onArtifactRemove: (artifactId: string) => void; onArtifactReveal: (relativePath: string, mimeType: string) => void; onKnowledgeOpen: (relativePath: string, mimeType?: string) => void; knowledgeQuery: string; knowledgeResults: KnowledgeResult[]; knowledgeStatus: string; onKnowledgeQueryChange: (value: string) => void; onKnowledgeSearch: () => void; locale: "zh" | "en" }) {
  ...
  const sectionMeta = isAssets ? `${artifactRows.length} ${ui.files}` : isKnowledge ? (locale === "en" ? "manifest + local index" : "清单 + 本地索引") : isCapabilities ? `${desktopCapabilities.length} ${locale === "en" ? "capabilities" : "项"}`  : ui.live;
...
type ProviderEditorDraft = { id: string; platformId: string; modelId: string; modelIds: string[]; baseUrl: string; apiKey: string; endpoint: string; queryEndpoint: string; workflows: RunningHubWorkflowRegistration[]; workflowEditingId: string; workflowName: string; workflowRemoteId: string; workflowSource: string; workflowJson: string; workflowCapability: RunningHubWorkflowCapability; workflowRequestKind: RunningHubWorkflowRequestKind; workflowSubmitPath: string; workflowQueryPath: string; workflowInputNodeId: string; workflowInputFieldName: string };
...
function DesktopProviderEditorModal({ target, profile, locale, onClose, onSave, onRemove, onDiscoverModels }: { target: ProviderEditorTarget; profile?: DesktopProviderConfig; locale: "zh" | "en"; onClose: () => void; onSave: (previousId: string | undefined, draft: ProviderEditorDraft, capability: ProviderCapability) => string | undefined; onRemove: (id: string) => void; onDiscoverModels: (provider: Pick<DesktopProviderConfig, "source" | "id" | "baseUrl" | "apiKey">, capability: ProviderCapability) => Promise<readonly string[]> }) {
  ...
  const save = () => {
    ...
    if (draft.workflowEditingId && !draft.workflowJson.trim()) {
      ...
      const updated = draft.workflowRequestKind === "ai-app" && draft.workflowCapability === "audio_transcription"
        ? createRunningHubAudioTranscriptionRegistration({ id: existing.id, appId: remoteWorkflowId, name: draft.workflowName.trim() || existing.name, sourceUrl: draft.workflowSource, inputNodeId: draft.workflowInputNodeId, inputFieldName: draft.workflowInputFieldName, submitPath: draft.workflowSubmitPath, queryPath: draft.workflowQueryPath })
        : { ...existing, remoteWorkflowId, name: draft.workflowName.trim() || existing.name, capability: draft.workflowCapability, source: { ...existing.source, ...(draft.workflowSource.trim() ? { url: draft.workflowSource.trim() } : {}) }, definitionHash: existing.definitionHash };
      ...
      try {
        ...
        if (draft.workflowRequestKind === "ai-app" && draft.workflowCapability === "audio_transcription") {
          const registration = createRunningHubAudioTranscriptionRegistration({ id: `${draft.id.trim() || target.capability}-${remoteWorkflowId}`, appId: remoteWorkflowId, name: draft.workflowName.trim() || "RunningHub ASR", sourceUrl: draft.workflowSource, inputNodeId: draft.workflowInputNodeId, inputFieldName: draft.workflowInputFieldName, submitPath: draft.workflowSubmitPath, queryPath: draft.workflowQueryPath });
  ...
  return <div className="settings-provider-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="settings-provider-modal" role="dialog" aria-modal="true" aria-labelledby="provider-editor-title">
    ...
    <div className="settings-provider-modal-grid">
      ...
      {platform?.source === "runninghub" ? <div className="settings-provider-workflow-registry"><strong>{locale === "zh" ? "账号工作流注册" : "Account workflow registry"}</strong><small>{locale === "zh" ? "普通工作流导入 ComfyUI API JSON；ASR 使用 RunningHub AI App 协议，运行时会按节点映射提交音频并读取文本结果。" : "Import ComfyUI API JSON for normal workflows. ASR uses the RunningHub AI App protocol to submit audio bindings and read text results."}</small>{draft.workflows.length ? <div className="settings-provider-workflow-list">{draft.workflows.map((workflow) => <div key={workflow.id}><span><b>{workflow.name}</b><small>{workflow.capability} · {workflow.remoteWorkflowId} · v{workflow.version}</small></span><span><button type="button" className="link-button" onClick={() => update({ workflowEditingId: workflow.id, workflowName: workflow.name, workflowRemoteId: workflow.remoteWorkflowId, workflowSource: workflow.source.url ?? "", workflowCapability: workflow.capability, workflowRequestKind: workflow.request?.kind ?? "workflow", workflowSubmitPath: workflow.request?.submitPath ?? "", workflowQueryPath: workflow.request?.queryPath ?? "", workflowInputNodeId: workflow.nodeBindings[0]?.nodeId ?? "2", workflowInputFieldName: workflow.nodeBindings[0]?.fieldName ?? "audio" })}>{locale === "zh" ? "编辑" : "Edit"}</button><button type="button" className="link-button danger" onClick={() => update({ workflows: draft.workflows.filter((item) => item.id !== workflow.id), workflowEditingId: draft.workflowEditingId === workflow.id ? "" : draft.workflowEditingId })}>{locale === "zh" ? "删除" : "Remove"}</button></span></div>)}</div> : null}<div className="settings-provider-workflow-form"><label>{locale === "zh" ? "注册类型" : "Registration type"}<select value={draft.workflowRequestKind} onChange={(event) => update({ workflowRequestKind: event.target.value as RunningHubWorkflowRequestKind, workflowCapability: event.target.value === "ai-app" ? "audio_transcription" : (draft.workflowCapability === "audio_transcription" ? "audio" : draft.workflowCapability) })}><option value="workflow">{locale === "zh" ? "ComfyUI 工作流" : "ComfyUI workflow"}</option><option value="ai-app">RunningHub AI App</option></select></label><label>{locale === "zh" ? "工作流名称" : "Workflow name"}<input value={draft.workflowName} onChange={(event) => update({ workflowName: event.target.value })} /></label><label>{locale === "zh" ? "工作流 ID / App ID" : "Workflow ID / App ID"}<input value={draft.workflowRemoteId} onChange={(event) => update({ workflowRemoteId: event.target.value })} placeholder="1999879555714347010" /></label>{draft.workflowRequestKind === "ai-app" ? <><label>{locale === "zh" ? "提交 Endpoint" : "Submit endpoint"}<input value={draft.workflowSubmitPath} onChange={(event) => update({ workflowSubmitPath: event.target.value })} placeholder="/openapi/v2/run/ai-app/{id}" /></label><label>{locale === "zh" ? "音频节点 ID" : "Audio node ID"}<input value={draft.workflowInputNodeId} onChange={(event) => update({ workflowInputNodeId: event.target.value })} placeholder="2" /></label><label>{locale === "zh" ? "音频字段名" : "Audio field name"}<input value={draft.workflowInputFieldName} onChange={(event) => update({ workflowInputFieldName: event.target.value })} placeholder="audio" /></label></> : <label>{locale === "zh" ? "ComfyUI API JSON（新增或重映射时填写）" : "ComfyUI API JSON (for new or remapped workflows)"}<textarea value={draft.workflowJson} onChange={(event) => update({ workflowJson: event.target.value })} placeholder='{"3":{"class_type":"CLIPTextEncode","inputs":{"text":""}}}' spellCheck={false} /></label>}<label>{locale === "zh" ? "能力类型" : "Capability"}<select value={draft.workflowCapability} onChange={(event) => update({ workflowCapability: event.target.value as RunningHubWorkflowCapability })}><option value="image">{locale === "zh" ? "图片" : "Image"}</option><option value="video">{locale === "zh" ? "视频" : "Video"}</option><option value="digital_human">{locale === "zh" ? "数字人" : "Digital human"}</option><option value="video_enhance">{locale === "zh" ? "视频高清化" : "Video enhance"}</option><option value="audio">{locale === "zh" ? "音频生成" : "Audio generation"}</option><option value="audio_transcription">{locale === "zh" ? "ASR 音频转字幕" : "ASR transcription"}</option></select></label></div></div> : null}
    </div>
...
export function App() {
  ...
  const workflowAgentOptions = useMemo<WorkflowDynamicOption[]>(() => localAgentGroups.flatMap((group) => group.cards.map((card) => ({ value: card.id, label: card.title, description: card.description }))), [localAgentGroups]);
  ...
  const audioTranscriptionProvider = useMemo(() => {
    const provider = Object.values(config.providers ?? {}).find((candidate) => candidate.source?.trim().toLowerCase() === "runninghub" && candidate.workflows?.some((workflow) => workflow.capability === "audio_transcription")) ?? providerForCapability(config, "audio");
    const transcriptionWorkflows = provider.workflows?.filter((workflow) => workflow.capability === "audio_transcription") ?? [];
    const transcriptionWorkflow = transcriptionWorkflows.length === 1 ? transcriptionWorkflows[0] : undefined;
    if (!transcriptionWorkflow?.remoteWorkflowId) return provider;
    // A provider may contain music, speech, and ASR registrations together.
    // Persist the ASR workflow as the template's selected model so the host
    // does not have to guess which account-owned workflow should run.
    ...
    void (async () => {
      try {
        ...
        const health = await tauriBridge.invoke<{ status: string }>("health");
         ...
         setRuntimeStatus(locale === "zh" ? "正在读取桌面启动结果：确认本地数据状态…" : "Reading desktop startup results: checking local data status…");
         const state = await tauriBridge.invoke<{ integrity: boolean; interruptedRuns?: number }>("local_state_status");
        ...
        setRuntimeStatus(health.status === "ok"
          ? (runtime.ready ? (locale === "zh" ? "运行环境就绪" : "Runtime ready") : (locale === "zh" ? "运行环境需要修复" : "Runtime needs repair"))
  ...
  async function loadDesktopVoices(provider?: DesktopProviderConfig) {
    const audioProvider = provider ?? providerForCapability(config, "audio");
    const response = await sendHostMessage({
      version: 1,
      type: "media.voices",
